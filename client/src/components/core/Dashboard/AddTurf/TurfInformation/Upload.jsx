...
const Upload = ({ name, label, register, errors, setValue }) => {
  ...
  const { editTurf, turf } = useSelector((state) => state.turf);
        ...
        image ? (
          ...
          <div className="flex flex-col space-y-2">
            <label className="text-sm text-richblack-5" htmlFor={label}>
              <div>
                Turf Image <sup className="text-red-700">*</sup>
              </div>
              ...
              <div className="bg-richblack-700 flex min-h-[250px] cursor-pointer items-center justify-center rounded-md border-2 border-dotted border-richblack-500">
                <div
                  ...
                  tabIndex={0}
                >
                  <input id={label} name={name} type="file" accept="image/*,.jpeg,.jpg,.png" tabIndex="-1"  {...register(name, { required: true })} onChange={handelonchange} className="hidden" />
            ...
            {
              errors.courseImage && (<span className='ml-2 text-xs tracking-wide text-red-700'>
